...
test('Test empty initial state of ImportedData', () => {
    expect(ImportedData.getRightsCategories()).toStrictEqual([]);
  });
...
test('Test types of ImportedData data structure elements', () => {
    ...
    isInstanceOf(rightsCat, RightsCategory);
    isInstanceOf(subrights, SubRight);
    isInstanceOf(orgs, Organization);
    isInstanceOf(learnmores, learnMore);
...
function isInstanceOf(lst, expectedClass) {
    let len = lst.length;
    for(var i = 0; i < len; i++) {
